...
(function(){
  ...
  function isHorizontalScrollbarClick(header, ev){
    try {
      if (!header || !ev || typeof ev.clientY !== 'number') return false;
      var rect = header.getBoundingClientRect && header.getBoundingClientRect();
      if (!rect) return false;
      var overflow = (header.scrollWidth || 0) > ((header.clientWidth || 0) + 1);
      if (!overflow) return false;
      var gutter = Math.max(0, (header.offsetHeight || 0) - (header.clientHeight || 0));
      if (gutter <= 0) return false;
      return ev.clientY >= (rect.bottom - gutter);
    } catch(_e){ return false; }
  }
  function findClickedTabLink(ev){
    try {
      var target = ev && (ev.target || ev.srcElement);
      var link = target && target.closest ? target.closest('ul[data-bs-toggle="tabs"] [data-bs-toggle="tab"]') : null;
      if (link) return link;
      var header = target && target.closest ? target.closest('ul[data-bs-toggle="tabs"]') : null;
      if (!header || isHorizontalScrollbarClick(header, ev)) return null;
      var links = getTabLinks(header);
      if (!links.length || typeof ev.clientX !== 'number' || typeof ev.clientY !== 'number') return null;
      for (var i = 0; i < links.length; i++){
  ...
  function ensureSegmentedShell(header){
    try {
      ...
      if (parent.classList && parent.classList.contains('hfx-seg-shell')) {
        header.__hfxSegmentedShell = parent;
        return parent;
      }
      var previousShell = header.__hfxSegmentedShell;
      var shell = document.createElement('div');
      shell.className = 'hfx-seg-shell';
      parent.insertBefore(shell, header);
      shell.appendChild(header);
      header.__hfxSegmentedShell = shell;
      if (previousShell && previousShell !== shell && !previousShell.contains(header)) {
  ...
  function initSegmentedHeader(header){
    try {
      if (!header) return;
      if (header.getAttribute('data-hfx-segmented-init') === '1') {
