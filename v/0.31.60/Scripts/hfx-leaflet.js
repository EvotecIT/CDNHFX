(function () {
  ...
  var __hfxLeafletDefaultAccentColor = 'rgb(var(--hfx-surface-accent-rgb, var(--tblr-primary-rgb, 6, 111, 209)))';
  ...
  function initOne(el) {
    try {
      ...
      for (var i = 0; i < markers.length; i++) {
        ...
        if (!layer && useCircle && L.circleMarker) {
          layer = L.circleMarker([m.lat, m.lng], {
            ...
            color: color || __hfxLeafletDefaultAccentColor,
            fillColor: color || __hfxLeafletDefaultAccentColor,
      ...
      for (var cIdx = 0; cIdx < circles.length; cIdx++) {
        ...
        try {
          var cColor = c.color || __hfxLeafletDefaultAccentColor;
      ...
      for (var plIdx = 0; plIdx < polylines.length; plIdx++) {
        ...
        try {
          var plOpts = {
            color: pl.color || __hfxLeafletDefaultAccentColor,
      ...
      for (var pgIdx = 0; pgIdx < polygons.length; pgIdx++) {
        ...
        try {
          var pgColor = pg.color || __hfxLeafletDefaultAccentColor;
      ...
      for (var gjIdx = 0; gjIdx < geoJsonLayers.length; gjIdx++) {
        ...
        try {
          var gjColor = gj.color || __hfxLeafletDefaultAccentColor;
  ...
  function init(root) {
    try {
      ...
      for (var i = 0; i < els.length; i++) {
        ...
        var lazy = true;
        try {
          var rawCfg = el.getAttribute('data-hfx-leaflet-config');
          if (rawCfg) {
            var parsedCfg = JSON.parse(rawCfg);
            if (parsedCfg && parsedCfg.lazyInit === false) lazy = false;
          }
        } catch (_cfg) { /* swallow */ }
