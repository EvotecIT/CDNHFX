  ...
  function createDeferredRecordStory(story) {
    ...
    item.className = 'hfx-monitoring-record-story is-' + (story.State || story.state || 'healthy');
    ...
    if (before || after) {
      ...
      if (before) transition.appendChild(createRecordStoryValue(after ? 'Before' : 'Previous value', before));
      if (before && after) {
      ...
      if (after) transition.appendChild(createRecordStoryValue(before ? 'After' : 'Value', after));
      item.appendChild(transition);
    }
    return item;
  }

  function appendGroupedRecordStories(container, recordStories, previewLimit) {
    var groups = [];
    ...
    groups.forEach(function (group) {
      ...
      section.className = 'hfx-monitoring-record-story-section';
  ...
  function createDeferredRecordRows(table, data, record, visibleMap, selected) {
    ...
    appendGroupedRecordStories(stories, record.Stories || record.stories || [], recordStoryPreviewLimit(explorer));
