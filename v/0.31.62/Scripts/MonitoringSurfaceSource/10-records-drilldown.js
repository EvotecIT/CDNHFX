  ...
  function recordStoryPreviewLimit(explorer) {
    var parsed = parseInt(explorer && explorer.getAttribute('data-hfx-monitoring-story-preview-limit') || '3', 10);
    return isFinite(parsed) && parsed > 0 ? parsed : 3;
  }

  function createRecordStoryValue(labelText, valueText) {
  ...
  function createDeferredRecordStory(story) {
    var presentation = (story.Presentation || story.presentation || 'value').toLowerCase();
    var item = document.createElement('article');
    item.className = 'hfx-monitoring-record-story is-' + presentation + ' is-' + (story.State || story.state || 'healthy');
    item.setAttribute('data-hfx-monitoring-record-story', 'true');
    item.setAttribute('data-hfx-monitoring-record-story-presentation', presentation);
    var head = document.createElement('div');
    head.className = 'hfx-monitoring-record-story-head';
    var kind = document.createElement('span');
    kind.className = 'hfx-monitoring-record-story-kind';
    kind.setAttribute('aria-hidden', 'true');
    kind.textContent = presentation === 'content' ? 'Content' : presentation === 'relationship' ? 'Relationship' : presentation === 'permission' ? 'Access' : 'Change';
    head.appendChild(kind);
    var action = document.createElement('strong');
    action.className = 'hfx-monitoring-record-story-action';
    action.textContent = story.Action || story.action || '';
    ...
    var context = story.Context || story.context || '';
    if (context) {
      var contextElement = document.createElement('div');
      contextElement.className = 'hfx-monitoring-record-story-context';
      contextElement.textContent = context;
      item.appendChild(contextElement);
    }

    var before = story.Before || story.before || '';
    var after = story.After || story.after || '';
    if (before || after) {
      var labels = presentation === 'content'
        ? { before: 'Previous value', after: 'Current value', singleBefore: 'Removed value', singleAfter: 'Added value' }
        : presentation === 'relationship'
          ? { before: 'Previous relationship', after: 'Current relationship', singleBefore: 'Removed relationship', singleAfter: 'Relationship' }
          : presentation === 'permission'
            ? { before: 'Previous access', after: 'Current access', singleBefore: 'Previous access', singleAfter: 'Current access' }
            : { before: 'Before', after: 'After', singleBefore: 'Previous value', singleAfter: 'Value' };
      var transition = document.createElement('div');
      transition.className = 'hfx-monitoring-record-story-transition' + (before && after ? '' : ' is-single-value');
      if (before) transition.appendChild(createRecordStoryValue(after ? labels.before : labels.singleBefore, before));
      if (before && after) {
        var arrow = document.createElement('span');
        arrow.className = 'hfx-monitoring-record-story-arrow';
        arrow.textContent = '→';
        transition.appendChild(arrow);
      }
      if (after) transition.appendChild(createRecordStoryValue(before ? labels.after : labels.singleAfter, after));
      item.appendChild(transition);
    }
    return item;
  }

  function appendGroupedRecordStories(container, recordStories, previewLimit, explorer) {
    var accordions = explorer && explorer.getAttribute('data-hfx-monitoring-story-disclosure') === 'accordions';
    var groups = [];
    (recordStories || []).forEach(function (story) {
      var label = story.Group || story.group || 'Changes';
      var group = groups.filter(function (candidate) { return candidate.label.toLowerCase() === label.toLowerCase(); })[0];
      if (!group) {
        group = { label: label, stories: [] };
        groups.push(group);
      }
      group.stories.push(story);
    });
    groups.forEach(function (group, groupIndex) {
      var section = document.createElement(accordions ? 'details' : 'section');
      section.className = 'hfx-monitoring-record-story-section' + (accordions ? ' is-accordion' : '');
      if (accordions && groupIndex === 0) section.open = true;
      var sectionHead = document.createElement(accordions ? 'summary' : 'div');
      sectionHead.className = 'hfx-monitoring-record-story-section-head';
      var heading = document.createElement('h4');
      heading.textContent = group.label;
      var count = document.createElement('span');
      count.className = 'hfx-monitoring-record-story-count';
      count.textContent = group.stories.length === 1 ? '1 change' : group.stories.length + ' changes';
      ...
      group.stories.slice(0, previewLimit).forEach(function (story) {
        visible.appendChild(createDeferredRecordStory(story));
      });
      section.appendChild(visible);

      var remaining = group.stories.length - previewLimit;
      if (remaining > 0) {
        var disclosure = document.createElement('details');
        disclosure.className = 'hfx-monitoring-record-story-more';
        ...
        group.stories.slice(previewLimit).forEach(function (story) {
          hidden.appendChild(createDeferredRecordStory(story));
        });
        disclosure.appendChild(hidden);
  ...
  function createDeferredRecordRows(table, data, record, visibleMap, selected) {
    ...
    var sourceKey = record.SourceKey || record.sourceKey || key;
    if (sourceKey !== key) row.setAttribute('data-hfx-monitoring-record-source-key', sourceKey);
    row.setAttribute('data-hfx-monitoring-record-title', title);
    ...
    var expanded = selected && (inlineRecordDetailsEnabled(explorer) || (drawer && !drawer.classList.contains('is-collapsed')));
    ...
    details.hidden = !selected || !inlineRecordDetailsEnabled(explorer);
    ...
    (record.Details || record.details || []).forEach(function (detail) {
      ...
      var groupKey = detail.GroupKey || detail.groupKey || '';
      if (groupKey) item.setAttribute('data-hfx-monitoring-detail-group', groupKey);
      var label = document.createElement('span');
    ...
    stories.className = 'hfx-monitoring-record-stories';
    appendGroupedRecordStories(stories, record.Stories || record.stories || [], recordStoryPreviewLimit(explorer), explorer);
    if (stories.childNodes.length) detailCell.appendChild(stories);
  ...
  function applyDeferredRecords(scope, table, query, filters, focusKey) {
    ...
    if (focusKey) {
      var focusIndex = records.findIndex(function (record) {
        return (record.Key || record.key || '') === focusKey;
      });
      if (focusIndex >= 0) currentPage = Math.floor(focusIndex / pageSize) + 1;
    }
  ...
  function revealDeferredRecord(table, key) {
    var data = deferredRecordData(table);
    if (!data || !(data.Records || data.records || []).some(function (record) {
      return (record.Key || record.key || '') === key;
    })) return null;

    var scope = scopeFor(table);
    applyDeferredRecords(scope, table, activeSearchQuery(scope, table), activeFilterTokens(scope, table), key);
    var row = Array.prototype.find.call(table.querySelectorAll('[data-hfx-monitoring-record-row]'), function (candidate) {
      return candidate.getAttribute('data-hfx-monitoring-record-row') === key;
    });
    if (row) return row;

    // A cross-view selection is explicit navigation: clear filters that hide the selected record.
    scope.querySelectorAll('[data-hfx-monitoring-search]').forEach(function (input) {
      if (surfaceAcceptsControl(table, input)) input.value = '';
    });
    scope.querySelectorAll('[data-hfx-monitoring-chip].is-active').forEach(function (chip) {
      var group = chip.closest ? chip.closest('[data-hfx-monitoring-chips]') : null;
      if (!surfaceAcceptsControl(table, group || chip)) return;
      var neutral = group ? group.querySelector('[data-hfx-monitoring-chip]:not([data-hfx-monitoring-filter]), [data-hfx-monitoring-chip][data-hfx-monitoring-filter=""]') : null;
      if (neutral) {
        setExclusiveActive(group.querySelectorAll('[data-hfx-monitoring-chip]'), neutral, 'is-active', 'aria-pressed');
      } else {
        chip.classList.remove('is-active');
        chip.setAttribute('aria-pressed', 'false');
      }
    });
    var explorer = table.closest('[data-hfx-monitoring-record-explorer]');
    if (explorer) explorer.querySelectorAll('[data-hfx-monitoring-column-filter]').forEach(function (input) { input.value = ''; });
    applyFilters(scope);
    applyDeferredRecords(scope, table, '', [], key);
    return Array.prototype.find.call(table.querySelectorAll('[data-hfx-monitoring-record-row]'), function (candidate) {
      return candidate.getAttribute('data-hfx-monitoring-record-row') === key;
    }) || null;
  }
