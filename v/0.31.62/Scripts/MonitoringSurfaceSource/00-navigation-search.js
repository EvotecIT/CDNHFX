  ...
  function scopeFor(element) {
    return element.closest('[data-hfx-monitoring-search-scope]') ||
      element.closest('[data-hfx-monitoring-page].is-active') ||
  ...
  function changePageSize(control) {
    var block = control && control.closest ? control.closest('[data-hfx-monitoring-page-block], .hfx-monitoring-table-block') : null;
    var surface = block ? block.querySelector('[data-hfx-monitoring-page-size]') : null;
    var pageSize = Number(control && control.value);
    if (!surface || !Number.isFinite(pageSize) || pageSize <= 0) return false;
    surface.setAttribute('data-hfx-monitoring-page-size', String(pageSize));
    setCurrentPage(surface, 1);
    applyFilters(scopeFor(control));
    return true;
  }
