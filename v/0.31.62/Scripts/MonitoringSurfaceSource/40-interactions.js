  function bind(root) {
    ...
    root.addEventListener('click', function (event) {
      ...
      if (recordDrawerToggle && root.contains(recordDrawerToggle)) {
        event.preventDefault();
        toggleRecordDrawer(recordDrawerToggle, { restoreFocus: event.detail === 0 });
        return;
      }
      ...
      if (recordToggle && root.contains(recordToggle)) {
        event.preventDefault();
        activateRecordRow(recordToggle.closest('tr'), { focusDrawer: event.detail === 0 });
        return;
      }
      ...
      if (correlationCell && root.contains(correlationCell)) {
        event.preventDefault();
        activateCorrelationCell(correlationCell, { focusDrawer: event.detail === 0 });
        return;
      }
    ...
    root.addEventListener('change', function (event) {
      var pageSize = event.target && event.target.closest
        ? event.target.closest('[data-hfx-monitoring-page-size-select]')
        : null;
      if (!pageSize || !root.contains(pageSize)) return;
