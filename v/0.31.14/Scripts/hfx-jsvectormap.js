(function () {
  ...
  function initOne(el, options, cfg) {
    try {
      ...
      try { bindVisibilityObserver(el); } catch (_vo) { /* ignore */ }

      function resize() { try { updateSizeFor(el); } catch (_e) { } }
  ...
  function init(elOrSelector, options, cfg) {
    ...
    var initCfg = cfg || {};
    var lazy = initCfg.lazyInit !== false;

    function boot() {
      var el = elOrSelector;
      try { if (typeof elOrSelector === 'string') el = document.querySelector(elOrSelector); } catch (_e) { /* ignore */ }
      return initOne(el, initOptions, initCfg);
    }

    if (lazy && window.htmlForgeXWhenVisible) {
      try {
        window.htmlForgeXWhenVisible(elOrSelector, function (node) { initOne(node, initOptions, initCfg); });
        return null;
  ...
  function elementSize(el) {
    try {
      ...
      var w = r && r.width ? r.width : (el.clientWidth || 0);
      var h = r && r.height ? r.height : (el.clientHeight || 0);
      return { w: w, h: h };
    } catch (_e) {
      return { w: 0, h: 0 };
    }
  ...
  function scheduleResizeRetry(el) {
    try {
      if (!el) return false;
      var retry = el.__hfx_jvm_resize_retry || 0;
      if (retry >= 3) return false;
      el.__hfx_jvm_resize_retry = retry + 1;
      var delay = 80 + (retry * 120);
      setTimeout(function () { try { updateSizeFor(el); } catch (_e) { } }, delay);
      return true;
    } catch (_e) {
      return false;
    }
  ...
  function updateSizeFor(el) {
    try {
      if (!el) return false;
      var map = el.__hfx_jvm_map;
      if (!map && el.id && window.htmlForgeXJsVectorMaps) map = window.htmlForgeXJsVectorMaps[el.id] || null;
      if (!map || typeof map.updateSize !== 'function') return false;
      var sz = elementSize(el);
      if (!sz.w || !sz.h) {
        return scheduleResizeRetry(el);
      }
      map.updateSize();
      return true;
    } catch (_e) { return false; }
  ...
  function refreshMaps(root) {
    try {
      if (!root) root = document;
      if (!root.querySelectorAll) return false;
      var els = root.querySelectorAll('[data-hfx-jvm=\"1\"]');
      if (!els || !els.length) return false;
      var ok = false;
      for (var i = 0; i < els.length; i++) {
        if (updateSizeFor(els[i])) ok = true;
      }
      return ok;
    } catch (_e) { return false; }
  }

  function resolveTargetSelector(el) {
    try {
      if (!el || !el.getAttribute) return '';
      ...
      sel = String(sel).trim();
      if (!sel) return '';
      if (sel[0] !== '#' && sel.indexOf('.') !== 0 && sel.indexOf('[') !== 0) return '';
      return sel;
    } catch (_e) { return ''; }
  }

  function bindBootstrapEvents() {
    if (document.__hfx_jvm_bs_bound) return;
    document.__hfx_jvm_bs_bound = true;

    document.addEventListener('shown.bs.tab', function (e) {
      try {
        var sel = resolveTargetSelector(e && e.target ? e.target : null);
        if (sel) {
          var panel = document.querySelector(sel);
          if (panel) {
            setTimeout(function () { refreshMaps(panel); }, 0);
            setTimeout(function () { refreshMaps(panel); }, 120);
            return;
          }
        }
      } catch (_e2) { /* ignore */ }
      try {
        setTimeout(function () { refreshMaps(document); }, 0);
      } catch (_e3) { /* ignore */ }
    });
    ...
    document.addEventListener('shown.bs.collapse', function (e) {
      try {
        ...
        if (tgt) {
          setTimeout(function () { refreshMaps(tgt); }, 0);
          setTimeout(function () { refreshMaps(tgt); }, 120);
        }
      } catch (_e4) { /* ignore */ }
    });
  }

  function bindVisibilityObserver(el) {
    try {
      if (!el || el.__hfx_jvm_vis_observer) return false;
        ...
        try {
          (entries || []).forEach(function (entry) {
            try {
              ...
              updateSizeFor(el);
              setTimeout(function () { updateSizeFor(el); }, 120);
            } catch (_e0) { /* ignore */ }
          });
        } catch (_e1) { /* ignore */ }
      }, { root: null, rootMargin: '0px', threshold: 0.01 });
      obs.observe(el);
      el.__hfx_jvm_vis_observer = obs;
      return true;
    } catch (_e) {
      return false;
    }
  ...
  function bindGlobalResize() {
    if (window.__hfx_jvm_resize_bound) return;
    window.__hfx_jvm_resize_bound = true;
    var pending = null;
    var last = 0;
    var delay = 120;
    function tick() {
      pending = null;
      last = Date.now();
      try { refreshMaps(document); } catch (_e) { }
    }
    window.addEventListener('resize', function () {
      try {
        var now = Date.now();
        if (pending) clearTimeout(pending);
        if (!last || now - last > delay * 2) {
          pending = setTimeout(tick, 10);
        } else {
  ...
  window.hfxJsVectorMap.refresh = refreshMaps;
  ...
  try { bindBootstrapEvents(); } catch (_e) { /* ignore */ }
  try { bindGlobalResize(); } catch (_e2) { /* ignore */ }
})();
