  ...
  function initSwitcher(el) {
    ...
    function clearActiveNavigation() {
      ...
      try {
        ...
        var dropdowns = document.querySelectorAll('.dropdown.show, .dropend.show');
    ...
    function setDropdownExpanded(container, expanded) {
      if (!container) return;

      try {
        if (container.classList) {
          container.classList[expanded ? 'add' : 'remove']('show');
        }
    ...
    function isSidebarDropdownContainer(container) {
      if (!container || !container.closest) return false;

      try {
        return !!container.closest('aside.navbar-vertical');
      } catch (e) {
        return false;
      }
    }
        ...
        if (node.classList && (node.classList.contains('dropdown') || node.classList.contains('dropend'))) {
          for (var i = 0; i < node.children.length; i++) {
            ...
            if (child && child.tagName === 'A' && child.classList.contains('dropdown-toggle')) {
              ...
              if (isSidebarDropdownContainer(node)) {
