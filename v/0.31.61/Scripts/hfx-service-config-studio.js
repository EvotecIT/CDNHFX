(function () {
  ...
  const observedStudios = new WeakSet();
  const jsonPanelState = new WeakMap();
  const configState = new WeakMap();
  const scrollableTabRefreshers = new WeakMap();
  const studioObservers = new WeakMap();
  const tabNavSelector = ".nav.nav-underline, [data-hfx-tabs-id]";
  ...
  function initStudio(studio) {
    ...
    if (initializedStudios.has(studio)) {
      initDynamicStudioContent(studio);
      observeDynamicStudioContent(studio);
      return;
    }
  ...
  function initDynamicStudioContent(studio) {
    initScrollableTabNavs(studio);
    if (typeof window.hfxTabsRefreshSegmented === "function") {
      window.hfxTabsRefreshSegmented(studio);
    }
    initLiveTables(studio);
    studio.querySelectorAll("[data-hfx-service-config-workbench]").forEach(initWorkbench);
  }

  function observeDynamicStudioContent(studio) {
    if (typeof MutationObserver !== "function") {
      return;
    }
    if (observedStudios.has(studio)) {
      ensureStudioRemovalObserver(studio, studioObservers.get(studio));
      return;
    }

    observedStudios.add(studio);
    const observer = new MutationObserver((mutations) => {
      const affectedNavs = new Map();
      let addedContent = false;
      let refreshSegmented = false;
      const markAffected = (nav, revealActive) => {
        affectedNavs.set(nav, affectedNavs.get(nav) === true || revealActive === true);
      };

      mutations.forEach((mutation) => {
        if (mutation.type === "attributes" && mutation.target instanceof Element) {
          const currentStyle = mutation.target.getAttribute("style") || "";
          if (mutation.attributeName === "style" && !/(?:display|visibility|flex-direction|direction)\s*:/i.test(`${mutation.oldValue || ""};${currentStyle}`)) {
            return;
          }

          const owner = mutation.target.closest(tabNavSelector);
          if (owner && studio.contains(owner)) {
            if (owner.classList.contains("hfx-nav-segmented")) {
              return;
            } else {
              const wasHiddenStyle = mutation.attributeName === "style" && /visibility\s*:\s*(hidden|collapse)/i.test(mutation.oldValue || "");
              const styleVisibilityChanged = mutation.attributeName === "style" && /(?:display|visibility)\s*:/i.test(`${mutation.oldValue || ""};${currentStyle}`);
              const ownerVisibilityChanged = mutation.target === owner && (mutation.attributeName === "hidden" || mutation.attributeName === "aria-hidden" || styleVisibilityChanged);
              const revealActive = ownerVisibilityChanged || (wasHiddenStyle && mutation.target.matches(".nav-link.active"));
              markAffected(owner, revealActive);
            }
          }
          return;
        }
        ...
        mutation.removedNodes.forEach((node) => {
          cleanupRemovedTabNavs(node);
          if (!(node instanceof Element)) {
            return;
          }

          const removedActive = node.matches(".nav-link.active") || Boolean(node.querySelector(".nav-link.active"));
          if (removedActive && mutation.target instanceof Element) {
            const owner = mutation.target.closest(tabNavSelector);
            if (owner && studio.contains(owner) && !owner.classList.contains("hfx-nav-segmented")) {
              markAffected(owner, true);
            }
          }
        });
        ...
        if (mutation.removedNodes.length > 0 && mutation.target instanceof Element) {
          ...
          if (owner && studio.contains(owner)) {
            if (owner.classList.contains("hfx-nav-segmented")) {
              refreshSegmented = true;
            } else {
              markAffected(owner, false);
            }
          }
        }
        ...
        mutation.addedNodes.forEach((node) => {
          ...
          addedContent = true;
          const addedActive = node.matches(".nav-link.active") || Boolean(node.querySelector(".nav-link.active"));
          const candidates = [];
          if (node.matches(tabNavSelector)) {
            candidates.push(node);
          }
          node.querySelectorAll(tabNavSelector).forEach((candidate) => candidates.push(candidate));
          const owner = node.closest(tabNavSelector);
          if (owner && studio.contains(owner)) {
            candidates.push(owner);
          }

          candidates.forEach((candidate) => {
            if (candidate.classList.contains("hfx-nav-segmented")) {
              refreshSegmented = true;
            } else {
              markAffected(candidate, addedActive);
            }
          });
        });
      });

      if (affectedNavs.size > 0) {
        const navs = Array.from(affectedNavs.keys());
        initScrollableTabNavs(studio, navs, false);
        navs.filter((nav) => affectedNavs.get(nav) === true)
          .forEach((nav) => scrollableTabRefreshers.get(nav)?.refresh?.());
      }
      if (refreshSegmented && typeof window.hfxTabsRefreshSegmented === "function") {
        window.hfxTabsRefreshSegmented(studio);
      }
      if (addedContent) {
        initLiveTables(studio);
    ...
    const state = { observer, removalObserver: null };
    studioObservers.set(studio, state);
    ensureStudioRemovalObserver(studio, state);
  }

  function ensureStudioRemovalObserver(studio, state) {
    if (!state || state.removalObserver || !document.documentElement?.contains(studio)) {
      return;
    }

    state.removalObserver = new MutationObserver(() => {
      if (document.documentElement.contains(studio)) {
        return;
      }

      cleanupRemovedTabNavs(studio);
      state.observer.disconnect();
      state.removalObserver?.disconnect();
      studioObservers.delete(studio);
      observedStudios.delete(studio);
    });
    state.removalObserver.observe(document.documentElement, { childList: true, subtree: true });
  }

  function cleanupRemovedTabNavs(node) {
    if (!(node instanceof Element)) {
      return;
    }
    ...
    node.querySelectorAll(tabNavSelector).forEach((nav) => navs.push(nav));
    navs.forEach((nav) => {
      const state = scrollableTabRefreshers.get(nav);
      state?.detach?.();
    });
  }
  ...
  function initScrollableTabNavs(studio, candidates, revealExisting = true) {
    const navs = candidates || Array.from(studio.querySelectorAll(tabNavSelector));
    navs.forEach((nav) => {
      if (!nav || !studio.contains(nav) || nav.classList.contains("hfx-nav-segmented")) {
        return;
      }
      if (initializedScrollableTabs.has(nav)) {
        const state = scrollableTabRefreshers.get(nav);
        state?.attach?.();
        const refresh = revealExisting ? state?.refresh : state?.refreshOverflow;
        refresh?.();
        if (refresh) {
      ...
      let pointerScrollBehavior = "";

      const isHorizontal = () => {
        const flexDirection = window.getComputedStyle(nav).flexDirection;
        return flexDirection === "row" || flexDirection === "row-reverse";
      };
      const hasOverflow = () => isHorizontal() && nav.getAttribute("data-hfx-tabs-overflowing") === "1";
      const isRtl = () => window.getComputedStyle(nav).direction === "rtl";
      const isRowReverse = () => window.getComputedStyle(nav).flexDirection === "row-reverse";
      const setState = (name, value) => nav.setAttribute(name, value ? "1" : "0");
      const updateOverflowState = () => {
        const horizontal = isHorizontal();
        if (nav.getAttribute("role") === "tablist" || nav.hasAttribute("aria-orientation")) {
        ...
        if (!horizontal) {
          ...
          nav.style.scrollBehavior = pointerScrollBehavior;
          nav.removeAttribute("data-hfx-scrollable-tabs");
          nav.removeAttribute("data-hfx-tabs-overflowing");
          nav.removeAttribute("data-hfx-tabs-can-left");
          nav.removeAttribute("data-hfx-tabs-can-right");
          return false;
        }

        nav.setAttribute("data-hfx-scrollable-tabs", "1");
        const navBounds = nav.getBoundingClientRect();
        const visibleLinks = Array.from(nav.querySelectorAll(".nav-link"))
          .filter((link) => {
            const style = window.getComputedStyle(link);
            return style.display !== "none" && style.visibility !== "hidden" && style.visibility !== "collapse";
          });
        const linkBounds = visibleLinks.map((link) => link.getBoundingClientRect())
          .filter((bounds) => bounds.width > 0 && bounds.height > 0);
        const leftmost = linkBounds.reduce((value, bounds) => Math.min(value, bounds.left), navBounds.left);
        const rightmost = linkBounds.reduce((value, bounds) => Math.max(value, bounds.right), navBounds.right);
        const contentStart = visibleLinks.reduce((value, link) => Math.min(value, link.offsetLeft), Number.POSITIVE_INFINITY);
        const contentEnd = visibleLinks.reduce((value, link) => Math.max(value, link.offsetLeft + link.offsetWidth), Number.NEGATIVE_INFINITY);
        const visibleSpan = Number.isFinite(contentStart) && Number.isFinite(contentEnd) ? Math.max(0, contentEnd - contentStart) : 0;
        const overflowing = visibleSpan > nav.clientWidth + 1;
        setState("data-hfx-tabs-overflowing", overflowing);
        setState("data-hfx-tabs-can-left", overflowing && leftmost < navBounds.left - 1);
        setState("data-hfx-tabs-can-right", overflowing && rightmost > navBounds.right + 1);
        return true;
      };
      const revealLink = (link) => {
        if (!link || !nav.contains(link) || !isHorizontal() || nav.scrollWidth <= nav.clientWidth + 1) {
          return;
        }
        ...
        if (linkBounds.left < navBounds.left + edgePadding) {
          nav.scrollLeft += linkBounds.left - navBounds.left - edgePadding;
        } else if (linkBounds.right > navBounds.right - edgePadding) {
          nav.scrollLeft += linkBounds.right - navBounds.right + edgePadding;
        }
      ...
      nav.addEventListener("wheel", (event) => {
        ...
        if (event.deltaMode === WheelEvent.DOM_DELTA_LINE) {
          const lineHeight = Number.parseFloat(window.getComputedStyle(nav).lineHeight);
          delta *= Number.isFinite(lineHeight) ? Math.max(16, lineHeight) : 16;
        } else if (event.deltaMode === WheelEvent.DOM_DELTA_PAGE) {
          delta *= Math.max(1, nav.clientWidth);
        }

        const previousScrollBehavior = nav.style.scrollBehavior;
        const previousScroll = nav.scrollLeft;
        nav.style.scrollBehavior = "auto";
        nav.scrollLeft += delta * (isRtl() !== isRowReverse() ? -1 : 1);
        nav.style.scrollBehavior = previousScrollBehavior;
        updateOverflowState();
        if (Math.abs(nav.scrollLeft - previousScroll) > 0.5) {
      ...
      nav.addEventListener("pointermove", (event) => {
        ...
        if (!moved && Math.abs(delta) > 4) {
          ...
          pointerScrollBehavior = nav.style.scrollBehavior;
          nav.style.scrollBehavior = "auto";
          if (typeof nav.setPointerCapture === "function") {
            try {
              nav.setPointerCapture(event.pointerId);
            } catch {
              // The pointer may already have ended or moved to another document.
            }
          }
        }
      ...
      nav.addEventListener("click", (event) => {
        ...
        window.requestAnimationFrame(() => revealLink(link));
      }, true);
      ...
      nav.addEventListener("hfx:tab-shown", (event) => {
        const link = event.target instanceof Element ? event.target.closest(".nav-link") : null;
        window.requestAnimationFrame(() => revealLink(link));
      });
      ...
      const observeLinks = () => {
        if (resizeObserver) {
          const currentLinks = new Set(nav.querySelectorAll(".nav-link"));
      ...
      const updateLayout = () => {
        ...
        if (updateOverflowState()) {
          revealLink(nav.querySelector(".nav-link.active"));
        }
      };
      const detach = () => {
        if (!attached) {
      ...
      const attach = () => {
        ...
        if (typeof ResizeObserver === "function") {
          resizeObserver = new ResizeObserver((entries) => {
            observeLinks();
            if (updateOverflowState() && entries.some((entry) => entry.target === nav)) {
              revealLink(nav.querySelector(".nav-link.active"));
            }
          });
      ...
      scrollableTabRefreshers.set(nav, { refresh: updateLayout, refreshOverflow, attach, detach });
      attach();
    });
  }
  ...
  async function saveFormConfig(studio, form) {
    ...
    const commitPaths = buildSubmitCommitPaths(detail.paths, formConfig, form);
  ...
  function refreshDependencies(studio) {
    ...
    studio.querySelectorAll("[data-hfx-service-config-field]").forEach((field) => {
      ...
      const expectedValues = readDependencyValues(field, expectedValue);
      const dependencySatisfied = !dependsOn || dependencyMatches(studio, dependsOn, expectedValues);
  ...
  function readDependencyValues(field, fallback) {
    const serialized = field.getAttribute("data-hfx-service-config-depends-values");
    if (!serialized) {
      return [fallback];
    }
  ...
  function dependencyMatches(studio, path, expectedValues) {
    const input = findFieldInput(studio, path);
    if (!input) {
      return false;
    }
    ...
    const normalizedExpectedValues = expectedValues.map((expectedValue) => normalize(expectedValue));
    if (Array.isArray(value)) {
      return value.map((item) => normalize(item)).some((item) => normalizedExpectedValues.includes(item));
    }

    return normalizedExpectedValues.includes(normalize(value));
  }
  ...
  function collectFields(scope) {
    ...
    root.querySelectorAll?.("[data-hfx-service-config-field-input]").forEach((input) => {
      const field = input.closest("[data-hfx-service-config-field]");
      if (field?.getAttribute("data-hfx-service-config-dependency-active") === "false" || isRepeaterTemplateElement(field)) {
        return;
      }
  ...
  function buildSubmitCommitPaths(paths, formConfig, form) {
    if (Array.isArray(paths) && paths.length > 0) {
      return paths;
    }

    const roots = new Set();
    form?.querySelectorAll?.("[data-hfx-service-config-field-input]").forEach((input) => {
      const field = input.closest("[data-hfx-service-config-field]");
      if (isRepeaterTemplateElement(field)) {
        return;
      }
  ...
  function bindAdapter(studio, adapter) {
    ...
    if (typeof adapter.saveForm === "function") {
      on(studio, "hfx:service-config-form-save-request", (event) => run("form-save", adapter.saveForm, event, () => {
        const paths = buildSubmitCommitPaths(event.detail?.paths || [], event.detail?.formConfig, event.detail?.form);
