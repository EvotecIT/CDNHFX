...
(function (global) {
    ...
    function protectExcelExportButtons(conf) {
        if (global.hfxDtExcel && typeof global.hfxDtExcel.protectExportButtons === 'function') {
            global.hfxDtExcel.protectExportButtons(conf);
        }
    }
    ...
    function reviveCallbacks(conf, policy) {
        try {
            ...
            protectExcelExportButtons(conf);
        } catch (_) { }
    }
    ...
    function tableSetForFooterFilters(api, $table) {
        try {
            var $ = global.jQuery || global.$; if (!$) return $table || $();
            return tableSetForFilters(api, $table).filter(function () {
                return $(this).closest(
                    'div.dt-scroll-head, div.dataTables_scrollHead'
                ).length === 0;
            });
        } catch (_) {
            return $table || (global.jQuery ? global.jQuery() : null);
    ...
    function footerFilters(tableSelector) {
        try {
            ...
            tableSetForFooterFilters(api, $t).each(function () {
                var $table = $(this);
    ...
    function safeExportData(api, exportOptions) {
        ...
        if (exportOptions && typeof exportOptions.customizeData === 'function') {
            exportOptions.customizeData(data);
        }
