(function(){
  ...
  function _isNaturalHeight(root){
    try{
      if(!root) return false;
      var mode = (root.getAttribute && root.getAttribute('data-hfx-height-mode')) || '';
      if(mode === 'natural') return true;
      if(root.classList && root.classList.contains('hfx-sw-report')) return true;
      var meta = __hfx_wizMeta.get(root) || null;
      if(meta && meta.heightMode === 'natural') return true;
      return false;
  ...
  function fixHeightForWizard(root){
    try{
      ...
      if(_isNaturalHeight(root)){
        $cont.css({'min-height':'0','height':'auto'});
        return;
      }
      ...
      function settleWizard(){
        try{
          try{
            ...
            if(opts && (typeof opts.autoAdjustHeight === 'boolean' || typeof opts.reportMode === 'boolean')){
              if(opts.autoAdjustHeight === false || opts.reportMode === true){
                meta.heightMode = 'natural';
                el.setAttribute('data-hfx-height-mode', 'natural');
              } else {
                meta.heightMode = 'fixed';
                el.setAttribute('data-hfx-height-mode', 'fixed');
              }
            }
    ...
    try{
      ...
      $wiz.on('shown.sw.hfxAjax', function(e, args){
        try{
          ...
          if(!cfg) return;
          var pane = cfg.id ? document.getElementById(String(cfg.id)) : null;
          var $pane = pane ? $(pane) : null;
