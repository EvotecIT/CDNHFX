...
(function (global) {
    ...
    function footerFilters(tableSelector) {
        try {
            ...
            if ($tfoot.find('tr.hfx-footer-filter').length) return;

            var $orig = $thead.find('tr').first();
            var $filter = $('<tr class=\'hfx-footer-filter\'/>');
            $orig.find('th').each(function (i) {
                var $th = $('<th/>').appendTo($filter);
                var title = ($(this).text() || '').trim();
                var searchable = true;
                try {
                    searchable = api.settings()[0].aoColumns[i].bSearchable !== false;
                } catch (_) { }
