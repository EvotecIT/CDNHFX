(function () {
  ...
  function resolveCssVar(value) {
    try {
      ...
      var out = s;
      for (var pass = 0; pass < 6 && out.indexOf('var(') !== -1; pass++) {
        var next = out.replace(/var\(([^()]+)\)/g, function (_match, inner) {
          try {
  ...
  function initOne(el) {
    try {
      ...
      function applyThemeColors() {
        try {
          var primary = resolveCssVar('rgb(var(--hfx-surface-accent-rgb, var(--tblr-primary-rgb, 6, 111, 209)))');
      ...
      if (heatPts.length && typeof globe.heatmapsData === 'function') {
        try { globe.heatmapsData([heatmap]); } catch (_hd) { /* swallow */ }
        if (typeof globe.heatmapPoints === 'function') globe.heatmapPoints(function (d) { return d && Array.isArray(d.points) ? d.points : []; });
  ...
  function init(root) {
    try {
      ...
      for (var i = 0; i < els.length; i++) {
        ...
        var lazy = true;
        try {
          var rawCfg = el.getAttribute('data-hfx-globe-config');
          if (rawCfg) {
