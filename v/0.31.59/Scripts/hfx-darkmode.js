(function () {
  ...
  function normalizeTheme(val) {
    ...
    if (v === 'dark-matrix' || v === 'matrix' || v === 'darkmatrix' || v === 'dot-matrix') return 'dark-matrix';
    return null;
  }
  ...
  function applyTheme(mode, token, selection) {
    ...
    try {
      var synced = document && document.querySelectorAll
        ? document.querySelectorAll('[data-hfx-theme-sync="bs-theme"]')
        : [];
      for (var i = 0; i < synced.length; i++) {
        synced[i].setAttribute('data-bs-theme', mode);
      }
    } catch (_sync) {
      // swallow
    }
  ...
  function applyResolvedTheme(selectionToken, key, persist) {
  ...
  function closeOwningDropdown(item) {
    try {
      ...
      try {
        var api = global.bootstrap && global.bootstrap.Dropdown;
        if (api && toggle) {
          var instance = api.getInstance(toggle) || api.getOrCreateInstance(toggle);
      ...
      if (toggle) {
        if (toggle.classList) toggle.classList.remove('show');
        toggle.setAttribute('aria-expanded', 'false');
        try { toggle.focus({ preventScroll: true }); } catch (_focus) { try { toggle.focus(); } catch (_focus2) { } }
      }
    } catch (_e) { }
  }
  ...
  function applyInitialTheme(storageKey) {
    ...
    try {
      ...
      if (urlTheme) {
        ...
        if (stored) {
          applyResolvedTheme(stored, storageKey);
          return;
        }

        var docSelection = normalizeTheme(getRootAttr('data-hfx-theme-selection')) ||
          normalizeTheme(getRootAttr('data-hfx-theme-default'));
        if (docSelection) {
          // Honor document-defined defaults without persisting them globally across unrelated pages.
          applyResolvedTheme(docSelection, storageKey, false);
          return;
        }

        applyResolvedTheme('auto', storageKey, false);
      }
    } catch (e) {
      // swallow
    }
